...
class Chatbot:
    ...
    def _get_groq_response(self, message, user_type):
        """Get response directly via httpx — no groq library needed"""
        if not GROQ_AVAILABLE:
            return "For detailed legal advice, please consult a verified lawyer on Zakoota."
        
        try:
            if user_type == 'lawyer':
                system_prompt = "You are a senior Pakistani lawyer with 20+ years experience. Provide accurate legal information with relevant Pakistan Penal Code sections, case laws, and practical advice."
            else:
                system_prompt = "You are Zing AI Legal Assistant, helping users with Pakistan laws. Provide accurate, simple explanations. Always cite relevant laws if known."
            
            message_lower = message.lower()
            if any(k in message_lower for k in ['detail', 'explain', 'step by step', 'full']):
                max_tokens = 350
            elif any(k in message_lower for k in ['brief', 'short', 'summary']):
                max_tokens = 100
            else:
                max_tokens = 200
